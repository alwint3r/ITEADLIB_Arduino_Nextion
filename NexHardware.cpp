 ...
 *
 */
bool NexHardware::recvRetNumber(uint32_t *number, uint32_t timeout)
{
    bool ret = false;
    uint8_t temp[8] = {0};
 ...
 *
 */
uint16_t NexHardware::recvRetString(char *buffer, uint16_t len, uint32_t timeout)
{
    uint16_t ret = 0;
 ...
 */
void NexHardware::sendCommand(const char* cmd)
{
    while (nexSerial->available())
    {
        nexSerial->read();
 ...
 *
 */
bool NexHardware::recvRetCommandFinished(uint32_t timeout)
{
    bool ret = false;
    uint8_t temp[4] = {0};
